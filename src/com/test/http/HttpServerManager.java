...
public class HttpServerManager {
    ...
    // 서버 생성 --> 서버 작동애 필요한 기본 정보, 즉 Context를 생성
    private void createServer(String host, int port) throws IOException {

        // HTTP SERVER 생성
        // 백로그 : 네트워크 포트에서 패킷을 쌓아두는 커널의 큐 크기, 0이면 시스템 기본값
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() { // 서버 종료 시 해 줄 일을 기술
...
class RootHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            sb.append("            <span>Method : " + exchange.getRequestMethod() + "</span><br>"); // GET, POST인지 확인
            sb.append("            <span>URI : " + exchange.getRequestURI() + "</span><br>"); // URI를 반환
            sb.append("            <span>PATH : " + exchange.getRequestURI().getPath() + "</span><br>"); // URI PATH를 반환
            sb.append("            <span>QueryString : " + exchange.getRequestURI().getQuery() + "</span><br>"); // URI PATH를 반환
            sb.append("    </body");
            sb.append("</html>");
            ...
            bb.get(content, 0, contentLength); // offset을 0으로 주면 값을 하나씩 가져옴 
            ...
            // 클라이언트에게 보낼 상태 코드와 Response, Body 사이즈를 설정 
